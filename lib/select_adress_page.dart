...
bool isShowPersons = false;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Center(child: Text("Thông tin khởi hành")),
        leading: IconButton(
      ...
      body: Padding(
        ...
        child: SingleChildScrollView(
          child: Column(
            ...
            children: [
              const Text('Điểm khởi hành/ điểm đến'),
              TextField(
              ...
              StreamBuilder<List<Userss>>(
                  ...
                  builder: (context, snapshot) {
                    return ElevatedButton.icon(
                        ...
                        icon: const Icon(Icons.send),
                        label: const Text("Tìm Hướng dẫn viên"));

                     
                  }),
   Visibility(child: Text('Danh sách các hướng dẫn viên ở khu vực')),
              persons()
            ],
          ),
        ),
  ...
  persons() {
    return StreamBuilder<List<Userss>>(
        ...
        builder: (context, snapshot) {
          ...
          // user = snapshot.data where location = locationController
          final userList = addresscontroller == null ? users : users.where((element) => element.location == addresscontroller.text).toList();

          return Flexible(
            child: Visibility(
              visible: isShowPersons,
